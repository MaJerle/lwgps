 ...
 * It is simple to show latitude, longitude and altitude for current GPS position
 */
...
int main(void) {
    ...
    /* Set end coordinates where we would like to go */
    ...
    while (1) {
        ...
        if (gpsRes == gpsNEWDATA) {                         /* Check for new pack of data */
            if (GPS.Valid) {                                /* Check valid GPS */
                ...
                /* Print values */
                printf("Distance from current to end positions: %f meters. Bearing we should move to reach end coordinate: %f degrees, bearing we are currently moving: %f degrees\r\n",
                    Distance.Distance, Distance.Bearing, GPS.Coarse
                );
                
                /* Compare actual moving with calculate where we should go */
                //if (GPS.Coarse == Distance.Bearing) ....
            }
        }
    }
}
